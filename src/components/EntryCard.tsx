...
export function EntryCard({ entry, compact = false }: EntryCardProps) {
  ...
  return (
    <Link
      href={`/browse/${slug}`}
      className="block p-4 sm:p-5 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] hover:border-[var(--color-border-hover)] hover:bg-[var(--color-surface-hover)] transition-colors duration-150 overflow-hidden"
    >
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4">
        <div className="min-w-0">
          <div className="flex items-center flex-wrap gap-2 mb-2">
            <h3 className="text-base sm:text-lg font-semibold text-[var(--color-foreground)] break-words">
              {frontmatter.title}
            </h3>
            {status === 'draft' && (
